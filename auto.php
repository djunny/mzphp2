...
return array(
	...
	'app_url' => 'http://localhost/mzphp/',
	
	// 应用的所在路径： 如: http://www.domain.com/bbs/
	'app_dir' => $app_dir,
	
	// CDN 缓存的静态域名，如 http://static.domain.com/
	'static_url' => 'http://localhost/mzphp/',
	
	// 模板使用的目录，按照顺序搜索，这样可以支持风格切换,结果缓存在 tmp/bbs_xxx.htm.php
	'view_path' => array(MZPHP_PATH.'templates/'), 
	
	// 数据模块的路径，按照数组顺序搜索目录
	'model_path' => array(MZPHP_PATH.'model/'),
...
define('MZPHP_PATH', str_replace('\\', '/', dirname(__FILE__)).'/');

if(!($conf = include './conf/conf.php')) {
	exit('config file not exists');
}
// 框架的物理路径
define('FRAMEWORK_PATH', MZPHP_PATH.'xiunophp/');
