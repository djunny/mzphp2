...
class HTML_Safe 
{
    ...
    private $white_value;
     private $white_css;
    function HTML_Safe($white_tag, $white_attr, $white_value, $white_css) 
    {
    ...
    function _writeAttrs ($attrs) 
    {
        if (is_array($attrs)) {
            foreach ($attrs as $name => $value) {
                ...
                // 白名单属性
                if(!in_array($name, $this->white_attr)) {
                	continue;
                // 分析css
                } elseif($name == 'style') {
        		...
        		/*
        			array(
        				'embed'=>array(
        					'width'=>array('range', 400, array(100, 800)),
        					'height'=>array('range', 400, array(100, 800)),
        					'allowsscriptaccess'=>array('list', 'on', array('on')),
        					'color'=>array('pcre', '#000000', array('#^\w+$#', '^rgb(\d+,\d+,\d+)$')),
        				)
        			);
        		*/
        		foreach($this->white_value[$name] as $v) {
        			if($v[0] == 'range') {
...
class HTML_White {
	...
	private $_liStack = array();
        ...
        function __construct($white_tag, $white_attr, $white_value, $white_css) {
	        $this->white_tag = $white_tag;
	        $this->white_attr = $white_attr;
	        $this->white_css = $white_css;
	        $this->white_value = $white_value;
		...
		foreach ($attrs as $name => $value) {
			...
			if(!in_array($name, $this->white_attr)) {
				...
				/*
        			...
        			*/
				preg_match_all('\s*([\w\-]+)\s*:\s*(.+?)\s*;?\s*', $value, $m);
				$csskv = is_array($m[1]) && is_array($m[2]) ? array_combine($m[1], $m[2]) : array();
				foreach($csskv as $cssname=>&$cssvalue) {
					if(!isset($this->white_css[$cssname])) continue;
					foreach($this->white_css[$cssname] as $v) {
		        			if($v[0] == 'range') {
		        				if($cssvalue > $v[2] && $cssvalue < $v[2]) $cssvalue = $v[1];
		        			} elseif($v[0] == 'list') {
	...
	private function _closeHandler(&$parser, $name) {
...
class xn_html_safe {
	public static function filter($doc) {
		...
		// 安全的 src
		// 安全的 href
		$safehtml = new HTML_Safe($white_tag, $white_attr, $white_value, $white_css);
